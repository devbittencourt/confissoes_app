...
export default function ReportPage() {
  ...
  const perguntasPorMandamento = { ...perguntasIniciais.dez, ...perguntasIniciais.igreja }
  const relatorio = useRelatorio(exame.state, perguntasPorMandamento)
  return (
    <div className="relative flex h-full min-h-screen w-full flex-col overflow-x-hidden max-w-md mx-auto shadow-2xl bg-background-light dark:bg-background-dark font-display">
      <div className="flex items-center bg-background-light dark:bg-background-dark p-4 pb-2 justify-between sticky top-0 z-50 border-b border-gray-200 dark:border-[#282e39]">
        ...
        <div className="flex items-center justify-end gap-2">
          <button className="flex items-center justify-center rounded-lg h-10 w-10 text-gray-900 dark:text-white hover:bg-gray-200 dark:hover:bg-[#282e39] transition-colors"><span className="material-symbols-outlined">ios_share</span></button>
        </div>
      </div>
      <div className="flex-1 flex flex-col p-4 gap-6">
        ...
        <div>
          ...
          <div className="bg-white dark:bg-[#1c212b] rounded-xl border border-gray-200 dark:border-[#3b4354] overflow-hidden shadow-sm">
            {relatorio.itensParaConfessar.length === 0 && (
              <div className="p-4 text-sm text-gray-500 dark:text-[#9da6b9]">Nenhum item marcado ainda.</div>
            )}
            {relatorio.itensParaConfessar.map((item) => (
              <div key={item.perguntaId} className="flex items-start justify-between p-4 border-b border-gray-100 dark:border-[#282e39]">
                <div className="flex items-start gap-3"><div className="mt-0.5 h-2 w-2 rounded-full bg-red-500 shrink-0"></div><div className="flex flex-col"><p className="text-gray-800 dark:text-gray-200 text-sm font-medium leading-normal">{item.texto}</p></div></div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2 bg-gradient-to-br from-primary/20 to-primary/5 dark:from-primary/20 dark:to-[#1c212b] rounded-xl p-4 border border-primary/20 flex items-center justify-between">
            <div><p className="text-primary text-xs font-bold uppercase tracking-wider mb-1">Maior Luta</p><h3 className="text-gray-900 dark:text-white text-xl font-bold">{relatorio.maiorLuta || '—'}</h3><p className="text-gray-500 dark:text-[#9da6b9] text-xs mt-1">Mandamento com maior recorrência</p></div>
